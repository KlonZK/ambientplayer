...
local PATH_CONFIG = widget.PATH_CONFIG --x
...
local TMP_INUSE_FILENAME =  widget.TMP_INUSE_FILENAME --x
--local MAPCONFIG_FILENAME = widget.MAPCONFIG_FILENAME
--local SOUNDS_ITEMS_DEF_FILENAME = widget.SOUNDS_ITEMS_DEF_FILENAME
--local SOUNDS_INUSE_DEF_FILENAME = widget.SOUNDS_INUSE_DEF_FILENAME
...
local SOUNDS_INUSE_HEADER = [[-- Sounditem definitions. same format as gamedata/sounds.lua 
...
function ReloadSoundDefs()
	...
	if not (spLoadSoundDef(rpath..TMP_INUSE_FILENAME)) then
		Echo("failed to load temp file:\n "..rpath..TMP_INUSE_FILENAME) return false end
...
function WriteLocalConfig(path, file)
	if (WriteTable(config, path..file, MAPCONFIG_HEADER)) then
...
function WriteOptions(path, file)
	if (WriteTable(options, path..file, OPTIONS_HEADER)) then
		Echo("saved local config to "..path..file)
	else Echo("failed to save local config :(") return false end	
